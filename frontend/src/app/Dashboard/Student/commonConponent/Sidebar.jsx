...
const Sidebar = () => {
  ...
  return (
    <div className={`${"side_bar"} ${'px-3 pb-4 pt-2 d-flex flex-column vh-100'}`}>
     ...
     ̀ {/* <Link href={`/Dashboard/Student/Fee/${id[4]}`} className={`custom_btn w-100 rounded-3 py-2 px-3 mt-2 d-flex justify-content-between align-items-center ${para == `/Dashboard/Student/Fee/${id[4]}` ? "active" : ""}`}><span><FontAwesomeIcon className='me-1' icon={faSackDollar} /> Fees</span> <FontAwesomeIcon className='text-dark' icon={faChevronRight} /></Link> */}
